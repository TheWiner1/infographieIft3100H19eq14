// IFT3100H19_Lighting/rendererDynamicLight.cpp
...
void RendererDynamicLight::setup()
{
  ofSetFrameRate(60);
  ofEnableDepthTest();
  ...
  teapot.loadModel("/home/win/Bureau/of_v0.10.1_linux64gcc6_release/apps/infographieIft3100H19eq14/bin/data/teapot.obj");
  iscube= isteapot=issphere=false;
  // initialisation de la scène
  reset();
}

void RendererDynamicLight::reset()
{
...
void RendererDynamicLight::update()
{
  ofPushMatrix();
  ...
  if (is_active_light_directional)
  {
    ...
    light_directional.setPosition(dir_posX, dir_posY + 60, camera_offset* 0.75f * dir_posZ);
  ...
  if (is_active_light_point)
  {
    ...
    light_point.setPosition(po_posX, po_posY, camera_offset * 0.75f * po_posZ);
  }
  ...
  if (is_active_light_spot)
  {
   ...
   // light_spot.setPosition (center_x, center_y - 75.0f, camera_offset * 0.75f);
    light_spot.setPosition (spo_posX, spo_posY -75.0f, camera_offset *0.75f* spo_posZ);
  }

  ofPopMatrix();
}

void RendererDynamicLight::draw()
{
  //ofBackground(0);
...
//        ofDrawBox(0, 0, 0, scale_cube);
        ofDrawBox(cub_posX, cub_posY, cub_posZ, scale_cube);
    ...
    ofPushMatrix();
      ...
      ofDrawSphere(sphe_posX, sphe_posY, sphe_posZ, scale_sphere);
      // désactiver le matériau
      material_sphere.end();
...
void RendererDynamicLight::lighting_on()
{
...
void RendererDynamicLight::lighting_off()
{
  ofSetGlobalAmbientColor(ofColor(150, 150, 150));
...
float RendererDynamicLight::oscillate(float time, float frequency, float amplitude)
{
